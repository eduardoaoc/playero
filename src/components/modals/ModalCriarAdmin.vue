  ...
  <div v-if="open" class="modal-overlay" @click="handleOverlayClick">
    <div class="modal-card" @click.stop>
      <header class="modal-header">
        <div>
          <p class="modal-eyebrow">Novo administrador</p>
          <h3 class="modal-title">Criar novo administrador</h3>
        </div>
      ...
      <form class="modal-body" @submit.prevent="handleSubmit">
        <div class="form-grid">
          ...
          <label class="form-field" :class="{ 'has-error': showPasswordError }">
            ...
            <small v-if="showPasswordError" class="form-error">Informe a senha.</small>
          </label>
          ...
          <label class="form-field form-field--toggle">
            <span>Status</span>
            <div class="toggle">
              <input id="admin-status" v-model="form.isActive" type="checkbox" />
        ...
        <div class="modal-actions">
          ...
          <button class="dash-action dash-action--primary" type="submit" :disabled="isSaving">
            ...
            <span v-else>Criar administrador</span>
          </button>
        </div>
      </form>
    </div>
...
const showPasswordError = computed(() => isSubmitted.value && !form.password.trim());
const showPasswordConfirmationError = computed(() =>
  isSubmitted.value && (!form.passwordConfirmation.trim() || form.password !== form.passwordConfirmation),
);
...
const handleSubmit = () => {
  ...
  if (
    ...
    showPasswordConfirmationError.value
  ) {
    return;
  }
